            ...
            if _show_results:
                ...
                if 'opt_scripts' in df_opt.columns and 'baseline_scripts' in df_opt.columns:
                    ...
                    st.markdown(
                        ...
                        f"<div style='background:{_sc_surf};border-radius:10px;padding:.6rem .8rem;"
                        f"border:1.5px solid {_sc_bdr}'>"
                        f"<div style='font-size:.6rem;color:{_sc_mu};font-weight:600;text-transform:uppercase;letter-spacing:.07em'>Total TRx</div>"
                        f"<div style='font-size:1.15rem;font-weight:800;color:{_sc_acc}'>{_opt_trx:,.0f}</div>"
                        f"<div style='font-size:.72rem;color:{'#057A55' if _incr_trx>=0 else '#C81E1E'};font-weight:500'>"
                        f"{'▲' if _incr_trx>=0 else '▼'} {abs(_incr_trx):,.0f} incremental</div>"
                        f"</div>"
                        f"<div style='background:{_sc_surf};border-radius:10px;padding:.6rem .8rem;"
                        f"border:1.5px solid {_sc_bdr}'>"
