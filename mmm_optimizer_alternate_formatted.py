...
REQUIRED = [
    'channel', 'total_spend', 'total_sales', 'coefficient',
...
def revenue_from_spend(spend: float, row: pd.Series) -> float:
    """Revenue calculation — curve type determined by row's type_transformation."""
    ...
    hill_p = None
    if str(tform).strip().lower() == 'hill' and pd.notnull(row.get('hill_ec50', np.nan)):
...
def mroi(spend: float, row: pd.Series, h_rel: float = 1e-4) -> float:
    """Instantaneous marginal ROI (derivative-based). Matches Excel mROI closely."""
    h = max(abs(spend) * h_rel, 1.0)
    return (revenue_from_spend(spend + h, row) -
            revenue_from_spend(spend - h, row)) / (2 * h)

def mroi_excel_style(spend: float, row: pd.Series) -> float:
    ...
    rev_hi = revenue_from_spend(spend,   row)
    rev_lo = revenue_from_spend(sp_lo,   row)
    d_sp   = spend - sp_lo
...
def total_profit(spends: np.ndarray, df: pd.DataFrame) -> float:
    return sum(
        revenue_from_spend(spends[i], df.iloc[i]) - spends[i]
        for i in range(len(spends))
    )

def total_revenue(spends: np.ndarray, df: pd.DataFrame) -> float:
    return sum(revenue_from_spend(spends[i], df.iloc[i]) for i in range(len(spends)))

def channel_roas_weighted(spends: np.ndarray, df: pd.DataFrame) -> float:
    total = 0.0
    for i in range(len(spends)):
        s = max(float(spends[i]), EPS)
        r = revenue_from_spend(s, df.iloc[i])
        total += r / s
    return total
...
def optimize_budget_gekko(df: pd.DataFrame,
    ...
    Supports per-channel Power / Log / Hill curves via type_transformation.
    """
    ...
    tform_arr = [str(r.get('type_transformation', 'power')).strip().lower() for r in rows]

    hill_ec50_arr = np.array([float(r.get('hill_ec50', 0.0)) if pd.notnull(r.get('hill_ec50', np.nan)) else 0.0 for r in rows])
    hill_slope_arr = np.array([float(r.get('hill_slope', 1.0)) if pd.notnull(r.get('hill_slope', np.nan)) else 1.0 for r in rows])
    hill_maxr_arr = np.array([float(r.get('hill_max_response', 0.0)) if pd.notnull(r.get('hill_max_response', np.nan)) else 0.0 for r in rows])
    ...
    # ----------------------------------------------------------
    # Objective functions — per-channel curve type
    # ----------------------------------------------------------
    def revenue_term(i):
        aps_expr = float(aps0_arr[i]) * x_v[i]
        af = float(adjf_arr[i])
        if tform_arr[i] == 'log':
            return af * float(coef_arr[i]) * m.log(1.0 + aps_expr ** float(alpha_arr[i])) * float(segs_arr[i]) * float(npu_arr[i])
        elif tform_arr[i] == 'hill' and hill_ec50_arr[i] > 0:
            sl = float(hill_slope_arr[i])
            ec = float(hill_ec50_arr[i])
            mx = float(hill_maxr_arr[i])
            return af * mx * (aps_expr ** sl) / (ec ** sl + aps_expr ** sl)
        else:
            return af * float(coef_arr[i]) * (aps_expr ** float(alpha_arr[i])) * float(segs_arr[i]) * float(npu_arr[i])
...
def optimize_budget_slsqp(df: pd.DataFrame, budget_constraint: float,
    ...
    if objective == "profit":
        neg_obj = lambda x: -total_profit(x, df)
    elif objective == "revenue":
        neg_obj = lambda x: -total_revenue(x, df)
    else:
        neg_obj = lambda x: -channel_roas_weighted(x, df)
...
def optimize_budget(df: pd.DataFrame, budget_constraint: float,
                    objective: str = "profit", method: str = "GEKKO") -> dict:
    """
    Main optimizer dispatcher.
    method: 'GEKKO' (default, recommended) | 'SLSQP' | 'Differential Evolution'
    """
    if method == "GEKKO":
        return optimize_budget_gekko(df, budget_constraint, objective)
    else:
        return optimize_budget_slsqp(df, budget_constraint, objective, method)
...
def find_optimal_budget(df: pd.DataFrame,
    ...
    Curve type is determined per-channel from type_transformation column.
    """
...
_ai_cfg_label = "🟢 AI Configuration — Connected" if _auth_ok else "⚙️ AI Configuration — Claude (Vertex AI)"
with st.expander(_ai_cfg_label, expanded=False):
...
with st.sidebar:
    ...
    uploaded = st.file_uploader("Upload CSV or Excel", type=["csv", "xlsx"],
                                  help="Required columns: channel, total_spend, total_sales, coefficient, type_transformation, alpha, lock_spend, lower_bound_pct, upper_bound_pct, total_segments, net_per_unit. Optional: total_activity (omit for spend-based curves)")
    ...
    with st.expander("📋 Schema reference", expanded=False):
        ...
        st.markdown(
            ...
            f"<b>Optional:</b> total_activity (omit for spend-based curves), hill_ec50, hill_slope, hill_max_response"
            f"</div>",
            unsafe_allow_html=True
        )
...
# ─── SPEND-BASED MODE DETECTION ──────────────────────────────────────────────
_SPEND_MODE = 'total_activity' not in df.columns or df['total_activity'].fillna(0).eq(0).all()
if _SPEND_MODE:
    df['total_activity'] = df['total_spend']
st.session_state['spend_mode'] = _SPEND_MODE
...
_has_hill_channels = (df['type_transformation'].str.strip().str.lower() == 'hill').any()
if _has_hill_channels and st.session_state['hill_df'] is None:
    with st.spinner("Computing Hill fits for hill-type channels..."):
        _hill_subset = df[df['type_transformation'].str.strip().str.lower() == 'hill']
        st.session_state['hill_df'] = _compute_hill_for_df(_hill_subset)
...
baseline_rev = [revenue_from_spend(float(r['total_spend']), r) for _, r in df.iterrows()]
...
df['baseline_mroi'] = [mroi(float(df.iloc[i]['total_spend']), df.iloc[i]) for i in range(len(df))]
...
if _SPEND_MODE:
    st.info(
        "**Spend-Based Mode** — `total_activity` column not found. "
        "Response curves are fitted directly against investment dollars. "
        "Coefficients and alpha must be estimated against spend (not activity)."
    )
...
with tab_rc:
    ...
    # ── Curve range input ──────────────────────────────────────────────────────
    _disp_hi_min = int(max(hi_pct * 100, 150))
    disp_hi = float(st.number_input(
        "Curve range (% of baseline investment)",
        min_value=_disp_hi_min,
        max_value=1000,
        value=_disp_hi_min,
        step=50,
        key=f"rc_disp_hi_{selected_ch}",
        help=f"How far to extend the response curve. Minimum {_disp_hi_min}% "
             f"(channel upper bound). Increase to explore higher spend scenarios."
    ))
    pct_grid = np.arange(0.0, disp_hi + 0.5, 1.0)
    spd_grid = base_sp * pct_grid / 100
    act_grid = (spd_grid / cpm) if cpm > 0 else spd_grid
    rev_grid = np.array([revenue_from_spend(s, rrow) for s in spd_grid])
    prof_grid = rev_grid - spd_grid
    mroi_grid = np.array([mroi(s, rrow) for s in spd_grid])
    ...
    curr_act   = base_act
    curr_avg_a = base_aps
    curr_rev   = revenue_from_spend(curr_sp, rrow)
    ...
    if _curve_flat:
        _flat_hint = "total_spend" if _SPEND_MODE else "total_activity and total_spend"
        st.warning(f"⚠️ Flat curve for {selected_ch} — check {_flat_hint} in your input.")
    ...
    with left_col:
        ...
        st.markdown(
            ...
            f"<div><div style='font-size:.58rem;color:{_c_mu2}'>{'Investment' if _SPEND_MODE else 'Activity'}</div>"
            f"<div style='font-size:.92rem;font-weight:700'>{fmt(curr_sp,'$') if _SPEND_MODE else fmt(curr_act)}</div></div>"
        ...
        if has_opt:
            ...
            st.markdown(
                ...
                f"<div><div style='font-size:.55rem;color:{_oa2}'>{'Investment' if _SPEND_MODE else 'Activity'}</div>"
                f"<b>{fmt(opt_sp,'$') if _SPEND_MODE else fmt(opt_act)}</b></div>"
    ...
    with right_col:
        ...
        if PLOTLY and not show_all:
            ...
            fig_rc.update_layout(
                ...
                xaxis=dict(
                    title=f"% of Baseline Investment  [{rrow['type_transformation'].upper()}]",
                    ticksuffix="%",
            ...
            for ri, (_, row) in enumerate(df.iterrows()):
                ...
                s_g    = b_sp * p_g / 100
                r_g    = np.array([revenue_from_spend(s, row) for s in s_g])
                ...
                b_rev  = revenue_from_spend(b_sp, row)
                b_prf  = b_rev - b_sp
                ...
                if opt_df_stored is not None and row['channel'] in opt_df_stored['channel'].values:
                    ...
                    o_rev = revenue_from_spend(o_sp, row)
                    o_pf  = o_rev - o_sp
        ...
        if show_table and not show_all:
            _tbl_hdr_col, _tbl_step_col = st.columns([0.7, 0.3])
            with _tbl_hdr_col:
                st.markdown("<div class='section-header' style='margin-top:.1rem'>Investment Response Table</div>",
                            unsafe_allow_html=True)
            with _tbl_step_col:
                _tbl_step_size = st.selectbox(
                    "Step size",
                    [1, 5, 10, 25],
                    index=2,
                    key="rc_table_step",
                    help="Display rows at this % interval (data is always computed at 1%)"
                )

            # ── Build table rows at user-selected steps, always include current & opt ──
            step_pcts = list(range(0, int(disp_hi) + 1, _tbl_step_size))
            ...
            for pct in all_pcts:
                ...
                h_     = max(sp * 1e-4, 1.0)
                if sp > 0:
                    rv_up  = revenue_from_spend(sp + h_, rrow)
                    rv_dn  = revenue_from_spend(sp - h_, rrow)
                ...
                tbl_rows.append({
                    ...
                    'Total_Activity': fmt(sp,"$") if _SPEND_MODE else _fmt(act, 0),
                    'Spend':         fmt(sp,"$"),
            ...
            tr_hdr    = (f"<tr style='background:{muted_bg};border-bottom:2px solid {border}'>"
                         ...
                         f"<th style='{th_style}'>{'Investment' if _SPEND_MODE else 'Total Activity'}</th>"
    ...
    if run_ai_curve:
        ...
        _ai_act_label = "Investment (spend-based curves)" if _SPEND_MODE else "Activity"
        _ai_act_value = fmt(float(rrow['total_spend']), '$') if _SPEND_MODE else f"{float(rrow['total_activity']):,.0f}"
        ai_prompt = (
...
with tab_rc:
    if show_all_curves and PLOTLY:
        ...
        _domain_label = "Investment domain" if _SPEND_MODE else "Activity domain"
        curve_header = (
            f"All channels · Profit response ({_domain_label})"
            if curve_type == "Profit Curve"
            else f"All channels · Revenue response ({_domain_label})"
        )
...
with tab_opt:
    ...
    with opt_results:
        ...
        _opt_key = (f"{obj_map[opt_objective]}|{method_map[opt_method]}|"
                    f"{opt_budget:.2f}|{budget_mode}|{_bounds_sig}")
...
def _render_launch_results(df, spends, lb_total, objective, method,
    ...
    df_out['launch_revenue'] = [revenue_from_spend(spends[i], df_out.iloc[i])
...
with tab_wi:
    ...
    with wi_chart_col:
        ...
        if "Uniform" in wi_mode:
            ...
            df_wi['wi_revenue'] = [revenue_from_spend(df_wi.iloc[i]['wi_spend'], df_wi.iloc[i])
        ...
        # ── MIX COMPARISON ────────────────────────────────────────────────────
            ...
            for sc_name, sc_spends in scenarios.items():
                ...
                df_sc['sc_revenue'] = [revenue_from_spend(df_sc.iloc[i]['sc_spend'], df_sc.iloc[i])
        ...
        # ── GOAL-SEEK ANALYSIS ────────────────────────────────────────────────
            ...
            st.markdown(
                ...
                f"Uses the uploaded model coefficients (alpha, Adj_Factor{'' if _SPEND_MODE else ', cost_per_mention'}) to "
            ...
            if "Single Channel" in gs_wi_mode:
                ...
                with gs_c2:
                    ...
                    def _gs_obj_val(spend_val):
                        rev_ch = revenue_from_spend(spend_val, _ch_row)
                        if "Portfolio Revenue" in gs_target_type:
                    ...
                    if _target_feasible:
                        ...
                        _gs_detail = pd.DataFrame([
                            ...
                            {'Metric': 'Channel mROI at solution',    'Baseline': f"{float(_ch_row['baseline_mroi']):.2f}×", 'Goal-Seek': f"{mroi(gs_solved_spend, _ch_row):.2f}×"},
                    ...
                    _gs_wi_prompt = (
                        ...
                        + (f"Required spend: {fmt(gs_solved_spend,'$')} ({gs_delta_pct:+.1f}% vs baseline {fmt(_base_sp,'$')})\n"
                           f"Channel mROI at solution: {mroi(gs_solved_spend, _ch_row):.2f}x\n"
            ...
            # ── BREAK-EVEN ANALYSIS ───────────────────────────────────────────
                ...
                for _, row in df.iterrows():
                    ...
                    _lo, _hi = 0.01, _hi_be
                    if revenue_from_spend(_hi, row) >= _hi:
                        be_sp = None
                    elif revenue_from_spend(0.01, row) <= 0.01:
                        be_sp = 0.01
                    else:
                        for _ in range(80):
                            _mid = (_lo + _hi) / 2
                            rev_mid = revenue_from_spend(_mid, row)
                            if rev_mid > _mid:
                                _lo = _mid
                            else:
                                _hi = _mid
                    ...
                    for _ in range(80):
                        ...
                        if (_hi2 - _lo2) < 0.01:
                            break
                    opt_sp = (_lo2 + _hi2) / 2
                    opt_rev  = revenue_from_spend(opt_sp, row)
...
with tab_ai:
    ...
    st.markdown(
        ...
        f"<span class='chip chip-gold'>Per-Channel Curves</span>",
        unsafe_allow_html=True)
    st.markdown("<div style='height:.5rem'></div>", unsafe_allow_html=True)
    ...
    if "Response Curve" in ai_mode:
        ...
        _t6_act_label = "Investment (spend-based)" if _SPEND_MODE else "Activity"
        _t6_act_value = '${:,.0f}'.format(float(rr['total_spend'])) if _SPEND_MODE else '{:,.0f}'.format(float(rr['total_activity']))
        _tab6_rc_prompt = (
            f"RESPONSE CURVE ANALYSIS\n"
            f"Channel: {ai_ch} | Type: {_desc}\n"
            f"Curve type: {rr['type_transformation']} | "
            ...
            f"1. What does this curve shape (alpha={float(rr['alpha']):.3f}, {rr['type_transformation']}) "
    ...
    # ═══════════════════════════════════════════════════════════════════════════
        ...
        _tab6_port_prompt = (
            ...
            f"Curve types: per-channel (from type_transformation column)\n\n"
    ...
    # ═══════════════════════════════════════════════════════════════════════════
        ...
        _tab6_diag_prompt = (
            ...
            f"3. Curve model assessment: are the per-channel type_transformation choices correct "
            f"for this channel mix? Which channels might benefit from a different curve type?\n"
